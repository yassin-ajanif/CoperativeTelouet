...
public partial class BonsReceptionListViewModel : ViewModelBase
{
    ...
    [ObservableProperty]
    private ObservableCollection<BonReceptionFournisseurListItemDto> _pageItems = [];

    [ObservableProperty]
    private BonReceptionFournisseurListItemDto? _selectedItem;
    ...
    public BonsReceptionListViewModel(IBonReceptionFournisseurService bons, IUserDialogService dialogs)
    {
        _bons = bons;
        _dialogs = dialogs;
    }
    ...
    public async Task LoadAsync()
    {
        try
        {
            ...
            var result = await _bons.GetBonsReceptionAsync(
            ...
            if (PageIndex > TotalPages)
            {
                ...
                result = await _bons.GetBonsReceptionAsync(
                    string.IsNullOrWhiteSpace(SearchText) ? null : SearchText,
                    TryParseDate(DateFromText),
                    TryParseDate(DateToText),
                    PageIndex,
            ...
            PageItems = new ObservableCollection<BonReceptionFournisseurListItemDto>(result.Items);
