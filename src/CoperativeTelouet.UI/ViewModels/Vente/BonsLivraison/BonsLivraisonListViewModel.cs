...
public partial class BonsLivraisonListViewModel : ViewModelBase
{
    ...
    [ObservableProperty]
    private ObservableCollection<BonLivraisonClientListItemDto> _pageItems = [];

    [ObservableProperty]
    private BonLivraisonClientListItemDto? _selectedItem;
    ...
    public BonsLivraisonListViewModel(IBonLivraisonClientService bons, IUserDialogService dialogs)
    {
        _bons = bons;
        _dialogs = dialogs;
    }
    ...
    public async Task LoadAsync()
    {
        try
        {
            ...
            var result = await _bons.GetBonsLivraisonAsync(
            ...
            if (PageIndex > TotalPages)
            {
                ...
                result = await _bons.GetBonsLivraisonAsync(
                    string.IsNullOrWhiteSpace(SearchText) ? null : SearchText,
                    TryParseDate(DateFromText),
                    TryParseDate(DateToText),
                    PageIndex,
            ...
            PageItems = new ObservableCollection<BonLivraisonClientListItemDto>(result.Items);
