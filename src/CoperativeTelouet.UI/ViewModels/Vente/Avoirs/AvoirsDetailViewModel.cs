...
public partial class AvoirsDetailViewModel : ViewModelBase
{
    ...
    [ObservableProperty]
    private ObservableCollection<FactureClientListItemDto> _factureOptions = [];

    [ObservableProperty]
    private FactureClientListItemDto? _selectedFacture;
    ...
    [ObservableProperty]
    private ObservableCollection<AvoirLigneItemViewModel> _lignes = [];

    [ObservableProperty]
    private AvoirLigneItemViewModel? _selectedLigne;
    ...
    public AvoirsDetailViewModel(
        IAvoirClientService avoirs,
    ...
    partial void OnSelectedFactureChanged(FactureClientListItemDto? value)
    {
        if (value is null) return;
        SelectedClient = ClientOptions.FirstOrDefault(c => c.Id == value.ClientId)
            ?? ClientOptions.FirstOrDefault();
    }

    [RelayCommand]
    public async Task LoadAsync(int? avoirId)
    {
        _editingId = avoirId;
        ...
        try
        {
            ...
            FactureOptions = new ObservableCollection<FactureClientListItemDto>(
                factures.Items.OrderByDescending(f => f.Date).ThenByDescending(f => f.Id));
    ...
    private async Task SaveAsync()
    {
        try
        {
            ...
            {
                await _avoirs.CreateAvoirAsync(new CreateAvoirClientDto(
                    Numero,
                    SelectedFacture.Id,
                    SelectedClient.Id,
                    DateDocument.Date,
            ...
            {
                await _avoirs.UpdateAvoirAsync(_editingId.Value, new UpdateAvoirClientDto(
                    SelectedFacture.Id,
                    SelectedClient.Id,
                    DateDocument.Date,
                    ttc,
    ...
    private void ReplaceLignes(IEnumerable<AvoirLigneItemViewModel> items)
    {
    ...
    {
        if (e.PropertyName is nameof(AvoirLigneItemViewModel.MontantHt)
            or nameof(AvoirLigneItemViewModel.MontantTtc)
            or nameof(AvoirLigneItemViewModel.Quantite)
            or nameof(AvoirLigneItemViewModel.PrixUnitaireHt)
            or nameof(AvoirLigneItemViewModel.Remise)
            or nameof(AvoirLigneItemViewModel.TauxTva))
            NotifyTotals();
    }
    ...
    {
        var prefs = await _columnVisibility.GetAsync(DocumentColumnKeys.AvoirClient);
    ...
    private Task SyncColumnVisibilityAsync() =>
        _columnVisibility.SaveAsync(
            DocumentColumnKeys.AvoirClient,
