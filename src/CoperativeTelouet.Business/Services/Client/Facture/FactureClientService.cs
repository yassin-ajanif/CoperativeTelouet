      ...
      IFactureClientService
{
    ...
    public Task<PagedResult<FactureClientListItemDto>> GetFacturesAsync(
        string? search = null,
        ...
        => RunLoggedAsync(nameof(GetFacturesAsync), () =>
        {
            ...
            return QueryPagedAsync(
                ...
                d => new FactureClientListItemDto(
                    d.Id,
                    d.Numero,
                    d.ClientId,
                    d.Client.Nom,
    ...
    public Task<FactureClientDto?> GetFactureByIdAsync(int id, CancellationToken cancellationToken = default)
        => RunLoggedAsync(nameof(GetFactureByIdAsync), async () =>
        {
            ...
            var lignes = await _lignes.FindAsync(l => l.FactureClientId == id, cancellationToken);
            return dto with
            {
                Lignes = lignes.Select(l => Mapper.Map<FactureClientLigneDto>(l)).ToList(),
            };
        });

    public Task<FactureClientDto> CreateFactureAsync(
